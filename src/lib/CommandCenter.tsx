...
export function CommandItem(props: CommandItemProps) {
  ...
  const { isSelected, matchesFilter, onSelectedUnmount, selectOption } =
    useCommandCenterCtx();
  ...
  const res = (
    own.href ? (
      <a
        ...
        role="option"
        aria-selected="false"
        onMouseEnter={(e) => selectOption(e.currentTarget)}
...
function getCommandText(element: HTMLElement) {
  // Prefer data-command-text attribute if available (excludes shortcuts)
  const commandTextElement = element.querySelector('[data-command-text]');
  if (commandTextElement) {
    return (commandTextElement as HTMLElement).textContent || (commandTextElement as HTMLElement).innerText;
  }
...
export function CommandList(
  ...
  const { listId } = useCommandCenterCtx();
