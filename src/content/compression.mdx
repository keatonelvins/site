...
title: "compression"
date: "2025-01-09"
description: "exploring the connections between compression, grokking, and information theory"
---
...
<figcaption>
  — Marcus Hutter, [the Hutter Prize](http://prize.hutter1.net/index.htm)
</figcaption>
</figure>
...
One lens we can understand language modeling through is the statistical next-token predictor, the autoregressive text generator extremely fitted to the distributional curve of natural language. This idea is actually super old, going back at least to the 1950's with an early Shannon work that references two key concepts to language modeling: _entropy_ and _redundancy_.
...
<figcaption>
  — Francois Chollet, [How I think about LLM prompt engineering](https://fchollet.substack.com/p/how-i-think-about-llm-prompt-engineering)
</figcaption>
</figure>

This framing seems relevant to pattern learning and better suited to thinking about grokking. If the model has simply memorized the training data, we can't expect interpolation to perform pretty well. For division mod 97, our program database would just be `return 15`, `return 71`, etc. If instead, we have learned a general rule – `lambda x, y, z: (x / y) % z` – or some nuanced abstractions, our model may perform surprisingly well, even perfectly, when tested on unseen samples. Small side-steps in a well-structured latent space can have a surprising power!
...
If we crack out our mechanistic interpretability toolkit, we can start talking about the underlying _circuits_ in the model actually responsible for computation. As [shown previously](https://arxiv.org/abs/2309.02390)[^5], there are multiple circuits that are learned during training and achieve strong performance. When we include weight decay, over time our model will learn to prefer circuits with high "efficiency", i.e. circuits that require less parameter norm to produce a given logit value. In [_Explaining grokking through circuit efficiency_](https://arxiv.org/abs/2301.05217)[^6], these results are supported; their interp identified three phases: memorization, then formation of circuits (before grokking phase), then cleanup (grokking), as the circuit both solves the task well and has lower weight than the memorized parameters, meaning weight decay encourages the network to shed the memorized solution.
...
<figcaption>
Mark Braverman, [Information Theory in Computer Science, Lecture 10](https://www.cs.princeton.edu/courses/archive/fall11/cos597D/L10.pdf)
</figcaption>
</figure>

Thankfully, there's a workaround. We can decide if the information is random or not by _seeing if removing it affects model performance_. We can use the difference in loss before/after compression as our rate-distortion metric! To do so, we can take the spectral entropy of each weight matrix (the effective rank) and measure the intrinsic dimension of the transformation parameterized by M. [Adding this term to our loss function speeds up time to grokking!](https://arxiv.org/abs/2412.09810)[^7] We've now moved from parameter-norm as our surrogate for complexity to actual compressability.

Past [other methods that use our understanding of grokking to speed up the phase-transition](https://arxiv.org/abs/2405.20233)[^8], I'm looking for future optimization techniques to speed-run memorization and more directly target generalization. At the limit, this looks like a lightweight (~1B?) reasoning model with little to no factual content memorized. A model that, given enough context, may start to do incredible things on your local machine.

<aside>
See also [Jan Nano-128](https://jan.ai/docs/jan-models/jan-nano-128) for a very early prototype of Karpathy's "cognitive core".
</aside>

[^1]: "[Grokking: Generalization Beyond Overfitting on Small Algorithmic Datasets](https://arxiv.org/abs/2201.02177)" (Power et al.)

[^2]: "[Anatomize Deep Learning with Information Theory](https://lilianweng.github.io/posts/2017-09-28-information-bottleneck/)" (Lilian Weng)

[^3]: "[Unifying Grokking and Double Descent](https://arxiv.org/abs/2303.06173)" (Davies et al.)

[^4]: "[Procedural Knowledge in Pretraining Drives Reasoning in Large Language Models](https://arxiv.org/abs/2411.12580)" (Ruis et al.)

[^5]: "[Progress measures for grokking via mechanistic interpretability](https://arxiv.org/abs/2309.02390)" (Nanda et al.)

[^6]: "[Explaining grokking through circuit efficiency](https://arxiv.org/abs/2301.05217)" (Varma et al.)

[^7]: "[The Complexity Dynamics of Grokking](https://arxiv.org/abs/2412.09810)" (DeMoss et al.)

[^8]: "[Grokfast: Accelerated Grokking by Amplifying Slow Gradients](https://arxiv.org/abs/2405.20233)" (Lee et al.)
